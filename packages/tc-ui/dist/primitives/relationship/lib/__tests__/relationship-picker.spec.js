...
const {
  h
} = require('preact'); // bizarely we get a fetch not found error unless this require is included
...
const {
  shallow,
  mount
} = require("../../../../test-helpers/enzyme-setup");
...
const getRelationshipList = wrapper => wrapper.find('ul.relationship-editor__list').first();
...
describe('relationship picker', () => {
  describe('initial states', () => {
    it("Can render the 'furniture'", () => {
      ...
      const wrapper = shallow(h(RelationshipPicker, props));
      expect(wrapper.is('div[data-component="relationship-picker"]')).toBe(true);
      expect(JSON.parse(wrapper.prop('data-props'))).toEqual({
        propertyName: 'testProperty'
      });
      expect(wrapper.children().length).toEqual(3);
      expect(wrapper.childAt(0).is('ul.relationship-editor__list.editable-relationships.o-layout__unstyled-element#ul-testProperty')).toBe(true);
      expect(wrapper.childAt(1).is('input[type="hidden"]#id-testProperty[name="testProperty"]')).toBe(true);
    ...
    describe('N-to-one field', () => {
      ...
      it('Can render a field with a single relationship', () => {
        ...
        expect(getRelationshipList(wrapper).childAt(0).is(Relationship)).toBe(true);
        expect(getRelationshipList(wrapper).childAt(0).props()).toMatchObject({
          value: rel,
          disabled: false,
          index: 0
        });
        expect(JSON.parse(getHiddenInput(wrapper).prop('value'))).toEqual(rel);
      });
      it('Can render a field with a single relationship with no name', () => {
        ...
        const wrapper = shallow(h(RelationshipPicker, props));
        expect(wrapper.childAt(0).is('ul.relationship-editor__list')).toBe(true);
        expect(getRelationshipList(wrapper).childAt(0).is(Relationship)).toBe(true);
        expect(getRelationshipList(wrapper).childAt(0).props()).toMatchObject({
          value: rel,
          disabled: false,
      ...
      it('Disables input when relationship selected', () => {
        ...
        expect(getAutoSuggest(wrapper).prop('inputProps')).toMatchObject({
          disabled: true,
          placeholder: 'Click "Remove" to replace the existing unique relationship'
        });
      });
    });
    describe('N-to-N field', () => {
      ...
      it('Can render a field with multiple relationships', () => {
        ...
        expect(getRelationshipList(wrapper).childAt(1).is(Relationship)).toBe(true);
        expect(getRelationshipList(wrapper).childAt(1).props()).toMatchObject({
          value: rel2,
          disabled: false,
          index: 1
        });
        expect(JSON.parse(getHiddenInput(wrapper).prop('value'))).toEqual([rel1, rel2]);
      });
    });
    describe('locking', () => {
  ...
  describe('adding a relationship', () => {
    ...
    it('fetches suggestions from the autocomplete api', async () => {
      ...
      expect(fetchMock.lastUrl()).toEqual('/autocomplete/MainType/name?q=a&parentType=ParentType&propertyName=testProperty');
      await fetchMock.flush();
      await new Promise(res => setTimeout(res, 50));
      wrapper.update(); // would like to test html output, but can't figure out how. Props is fine for now

      expect(getAutoSuggest(wrapper).prop('suggestions')).toEqual(suggestions);
    });
    it('does not suggest previously selected records', async () => {
      ...
      expect(getAutoSuggest(wrapper).prop('suggestions')).toEqual(suggestions.slice(2));
    });
    it('can select a selection when hasMany is false', async () => {
      ...
      expect(JSON.parse(getHiddenInput(wrapper).prop('value'))).toEqual(suggestions[2]);
    });
    it('can select a selection when hasMany is true', async () => {
      ...
      expect(getAutoSuggest(wrapper).prop('inputProps')).not.toMatchObject({
        disabled: true
      });
      expect(JSON.parse(getHiddenInput(wrapper).prop('value'))).toEqual([suggestions[2]]);
    });
    it('can select another selection when hasMany is true', async () => {
      ...
      expect(JSON.parse(getHiddenInput(wrapper).prop('value'))).toEqual([suggestions[0], suggestions[2]]);
    });
  });
  describe('removing a a relationship', () => {
    it('can remove a N-to-1 relationship', async () => {
      ...
      const wrapper = mount(h(RelationshipPicker, props));
      getRelationshipList(wrapper).children().first().find('.relationship-remove-button').simulate('click');
      await new Promise(res => setTimeout(res, 1));
      wrapper.update();
      expect(getRelationshipList(wrapper).children().length).toBe(0);
      expect(getHiddenInput(wrapper).prop('value')).toEqual('null');
    ...
    it('can remove a N-to-N relationship, leaving some remaining', async () => {
      ...
      expect(JSON.parse(getHiddenInput(wrapper).prop('value'))).toEqual([rel2]);
    });
