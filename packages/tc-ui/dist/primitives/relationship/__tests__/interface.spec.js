...
const {
  ...
  // , ViewComponent,
  parser
} = require("../server"); // const { withEditComponent } = require('../browser');
...
describe('relationship interfaces', () => {
  describe('<EditComponent />', () => {
    ...
    it.skip('Renders expected wrapper and children', () => {
      ...
      const wrapper = shallow(h(EditComponent, props));
      expect(wrapper.is('label.o-forms-field[data-biz-ops-type="relationship"]')).toBe(true);
  ...
  describe.skip('<ViewComponent />', () => {}); // Failure to attach edit components is fairly obvious to spot manually,
  ...
  describe('parser', () => {
    ...
    it('parses as JSON if exists', () => {
      expect(parser('{"test": true}')).toEqual({
        test: true
      });
    });
  });
