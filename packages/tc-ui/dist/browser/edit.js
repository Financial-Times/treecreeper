...
const {
  ...
  LargeText: {
    withEditComponent: attachDocumentEditor
  }
} = require("../primitives/browser");

const initDocumentEditors = () => {
  [...document.querySelectorAll('[data-type="Document"]')].forEach(attachDocumentEditor);
};

const initRelationshipSelectors = () => {
  [...document.querySelectorAll('[data-component="relationship-picker"]:not([data-disabled])')].forEach(attachRelationshipPicker);
...
const preventBadSubmission = () => {
  document.querySelector('form.o-layout__main').addEventListener('submit', ev => {
    const editorsInUnselectedState = [...document.querySelectorAll('[data-component="relationship-picker"][data-is-unresolved]')];

    if (editorsInUnselectedState.length) {
      const badFieldNames = editorsInUnselectedState.map(el => el.dataset.propertyName);
      window.alert(`\
...
const fieldValueCheckers = {
  ...
  boolean: field => !![...field.querySelectorAll('input')].some(input => input.checked),
  relationship: field => !![...field.querySelectorAll('input[type="hidden"]')].every(input => !!input.value)
};

const preventIncompleteSubmission = () => {
  const essentialFieldsFieldset = document.querySelector('.fieldset-minimumViableRecord');

  if (essentialFieldsFieldset) {
    document.querySelector('form.o-layout__main').addEventListener('submit', ev => {
      const essentialFields = [...essentialFieldsFieldset.querySelectorAll('[data-biz-ops-type]')];
      const unfilledFields = essentialFields.filter(field => {
        const type = field.dataset.bizOpsType;
        const checker = fieldValueCheckers[type] || fieldValueCheckers.default;
        return !checker(field);
      });

      if (unfilledFields.length) {
        const saveIncomplete = window.prompt(`Please fill out all fields required for the minimum viable record before creating a new record.
