...
class ApiClient {
  ...
  getHeaders() {
    const headers = typeof this.apiHeaders === 'function' ? this.apiHeaders(this.event) : this.apiHeaders;
    return {
  ...
  async extractErrorMessage(response) {
    ...
    try {
      ...
      errorMessage = errors.errors ? errors.errors.map(error => error.message).join('\n') : errors.error;
    } catch (err) {
  ...
  async logAndThrowError(response, props) {
    ...
    this.logger.error({
      error,
      event: 'API_ERROR'
    }, props, `Api call failed with status ${response.status}`);
    throw error;
  }
  ...
  async write(type, code, formData, method) {
    const response = await fetch(`${this.restApiUrl}/${type}/${encodeURIComponent(code)}?relationshipAction=replace`, {
      method,
      headers: this.getHeaders(),
  ...
  async delete(type, code) {
    const response = await fetch(`${this.restApiUrl}/${type}/${encodeURIComponent(code)}`, {
      method: 'DELETE',
      headers: this.getHeaders()
    });
  ...
  async read(type, code) {
    try {
      ...
      if (/Invalid type/.test(err.message)) {
        throw httpError(404, `"${type}" is not a valid type of record. Make sure you have entered the url correctly`);
      }
