...
const toGraphql = (propName, propDef, assignComponent) => assignComponent(propDef.type).graphqlFragment(propName, propDef);

const graphqlQueryBuilder = (type, assignComponent) => {
  return `
	query getStuff($itemId: String!) {
    ${type} (code: $itemId) {
    ${Object.entries(getType(type, {
    includeMetaFields: true
  }).properties).map(([propName, propDef]) => toGraphql(propName, propDef, assignComponent)).join('\n')}
  }
}
`;
};
