...
const getSchemaSubset = (event, type, isCreate = false) => {
  ...
  if (!properties) {
    return {
      schema: getType(type, { ...getTypeOptions,
    ...
    schema: { ...fullSchema,
      fieldsets: {
        [title]: {
          ...
          properties: Object.entries(fullSchema.properties).filter(([propName]) => propertyKeys.includes(propName)).reduce((fieldset, [propName, propDef]) => ({ ...fieldset,
            [propName]: propDef
          }), {})
        }
      }
