...
const getEditHandler = ({
  ...
  const displayForm = async (event, apiError) => {
    ...
    const templateData = { ...getSchemaSubset(event, type, isCreate),
      data: formData,
      type,
      code,
      assignComponent,
      isEdit: !!code,
      pageType: 'edit',
      pageTitle: formData.code ? `Edit ${type} ${formData.name}` : `New ${type}`
    };
  ...
  const handleForm = async event => {
    ...
    let method = 'PATCH'; // If a code is present then PATCH the existing record
    ...
    try {
      await apiClient.write(type, code, formDataToRest(type, formData), method);
    } catch (err) {
      const error = {
        type,
        code,
    ...
    return {
      status: 303,
      headers: {
        location: `/${type}/${encodeURIComponent(code)}?message=${type} ${code} was successfully updated&messageType=success`,
