...
const formatValue = (value, formatString) => value ? format(value, formatString) : null;
...
*/
format(timeInput, 'h:mm:ss a') !== 'Invalid Date' ? format(timeInput, 'h:mm:ss a') : timeInput;
...
module.exports = {
  ...
  toKebabCase: string => string.split(' ').map(str => str.toLowerCase()).join('-'),
  formatDateTime,
  LinkToRecord
};