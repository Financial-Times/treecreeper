...
describe('rest PATCH create', () => {
	...
	describe('writing disconnected records', () => {
		const patchKitchenSinkPayload = body =>
			patchHandler({
				type: 'KitchenSink',
				code: branchCode,
				body,
		...
		it('creates record with no body', async () => {
			const { status, body } = await patchKitchenSinkPayload();
		...
		it('creates record with properties', async () => {
			...
			const { status, body } = await patchKitchenSinkPayload(payload);
		...
		it('sets array data', async () => {
			...
			const { body, status } = await patchKitchenSinkPayload(payload);
		...
		it('throws 400 if code in body conflicts with code in url', async () => {
			await expect(
				patchKitchenSinkPayload({ code: 'wrong-code' }),
			).rejects.httpError({
				status: 400,
		...
		it('throws 400 if attempting to write property not in schema', async () => {
			await expect(
				patchKitchenSinkPayload({ notInSchema: 'a string' }),
			).rejects.httpError({
				status: 400,
				message:
		...
		it('throws if neo4j query fails', async () => {
			dbUnavailable();
			await expect(patchKitchenSinkPayload()).rejects.toThrow('oh no');
		});
	});
	...
	describe('creating relationships', () => {
		...
		const postSimpleGraphBranch = (body, query = {}) =>
			patchHandler({
				...
				code: branchCode,
				body,
				query: {relationshipAction: 'merge', ...query},
				metadata: getMetaPayload(),
			});
		...
		describe('rich relationships', () => {
			it('creates record with rich relationship to another record', async () => {
				...
				const { status, body } = await patchHandler({
					...
					query: { upsert: true, richRelationships: true, relationshipAction: 'merge' },
					metadata: getMetaPayload(),
				});
