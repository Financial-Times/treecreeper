...
describe('test schema properties',() => {
	it('excludes test properties by default', async () => {
		...
		expect(Object.keys(schema.getTypes({ grouped: true }))).toEqual(['prod']);
		expect(schema.getTypes({ grouped: true }).prod.types[0].name).toEqual('TypeA');
	})

		it('includes test properties on demand', async () => {
		const schema = new SDK({ schemaData: { schema: schemaFixture }, includeTestDefinitions: true})
		await schema.ready()
		...
		expect(Object.keys(schema.getTypes({ grouped: true }))).toEqual(['prod', 'test']);
		expect(schema.getTypes({ grouped: true }).prod.types[0].name).toEqual('TypeA');
		expect(schema.getTypes({ grouped: true }).test.types[0].name).toEqual('TypeB');
	})
