...
test:
	@if [ -z $(CI) ]; \
		...
		else TREECREEPER_SCHEMA_DIRECTORY=example-schema jest --config="./jest.config.js" "__tests__.*/*.spec.js" --testEnvironment=node --maxWorkers=2 --ci --reporters=default --reporters=jest --config="./jest.config.js"-junit; \
	fi
