	...
	Object.entries(enums).forEach(([name, { description, options }]) => {
		describe(name, () => {
			...
			if (Array.isArray(options)) {
				...
				it('has only string keys', () => {
					Object.keys(options).forEach(opt =>  expect(typeof opt).toBe('string'));
				});
				it('has only string values', () => {
					Object.values(options).forEach(opt =>  expect(typeof opt).toBe('string'));
				});
				it('has no numbers in keys', () => {
					Object.keys(options).forEach(opt => expect(opt).not.toMatch(/\d/));
				});
			}
		});
	});
