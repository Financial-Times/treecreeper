...
describe('Integration - GraphQL', () => {
		...
		const listFields = (fields = []) => fields
		...
		const getMocksForType = async (type, typeQuery) => {
			...
			`)
			.then(({data}) => data.__schema.types.find(({name}) => name.toUpperCase() === type.toUpperCase()).fields)
			.catch((error) => {
		...
		const addUniqueAttributes = (item) => Object.assign({}, item, {[UNIQUE_ATTRIBUTE_KEY]: UNIQUE_ATTRIBUTE_VALUE})

		const mapEnumFields = (item, fieldSchema) => Object.entries(item).reduce((result, [key, value]) => {
		...
		const populateDatabaseMocks = async (type, typeQuery) => {
			const {mocks, fields} = await getMocksForType(type, typeQuery)
				.then((result) => {
