...
*/
const mapKeyToField = (record) => (result, key) => Object.assign(result, { [key]: record._fields[record._fieldLookup[key]]});

/*
* Takes a neo4j response and maps the keys of each record to their corresponding field
*/
const parse = (response) => response.map(record => record.keys.reduce(mapKeyToField(record), {}));

/*
Array.filter callback
Find unique object within an array based on the supplied key
*/
const uniqueRowsByKey = (key) => (item, index, self) => index === self.findIndex((current) => ( current[key] === item[key] ));
