...
const findValidSupplierDiligenceSubmission = async (supplierId, surveyId) => {
	const timeSinceLastYear = new Date(
...
const cloneSupplierDiligenceSubmission = (supplierId, surveyList) => {
	surveyList.map(async surveyId => {
		const submissionsToClone = await getSubmissionsWithoutAnswers(
			supplierId,
			surveyId
		);
		if (submissionsToClone) {
			const previousSubmission = await findValidSupplierDiligenceSubmission(
				supplierId,
				surveyId
			);
				...
				submissionsToClone.forEach((submissionId, submissionIndex) => {
					Object.keys(previousSubmission).forEach((key, answerIndex) => {
						if (previousSubmission[key].question) {
							...
							submissionCloneQuery += `(submission${submissionIndex}) -[:HAS]-> (:SubmissionAnswer ${stringify(
								answer
							)}) -[:ANSWERS_QUESTION]-> (question${answerIndex}),`;
						}
					});
				});
				submissionCloneQuery = submissionCloneQuery.slice(0, -1);
...
const addSubmissionQuery = (contract, contractNode) => (
	query,
	survey,
	surveyIndex
) => {
	...
	query += ` MERGE (${contractNode}) -[:SUBMITS]-> (${submissionNode}:Submission ${stringify(
		submission
	)})
...
const addContractToQuery = (contract, contractIndex) => {
	...
	const contractQuery = ` MERGE (supplierNode)-[:SIGNS]->(${contractNode}:Contract ${contractInfo})
												ON CREATE SET ${contractNode}.dts = '${dts}'
												ON MATCH SET ${contractNode}.dts = ${contractNode}.dts + ',${dts}'`;
	const diligenceTypes = dt.some(dt => {
		dt.id === 'ra';
	})
		? dt
		: [{ id: 'ra' }, ...dt];
	return diligenceTypes.reduce(
		addSubmissionQuery(contract, contractNode),
		contractQuery
	);
};
...
const create = async (req, res) => {
	...
	initialQuery += ` MERGE (supplierNode)-[:SUBMITS]->(companyInfoNode:Submission ${stringify(
		companyInfo
	)})
	...
	const createQuery = req.body.contracts
		.map(addContractToQuery)
		.reduce((query, contractQuery) => query + contractQuery, initialQuery);
	try {
