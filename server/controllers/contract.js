...
const get = async (req, res) => {
	...
	const result = await db.run(query);
		...
		const answers = await Promise.all(cypher.res.uniquePropertiesByKey('answer', records)
...
const getFileUrls = async (answer) => {
	if(answer.type === 	'upload') {
		answer.value = await Promise.all(answer.value.map(valueToJSON).map(valueToFileUrl));
	}
	return answer;
};
