...
const get = async (req, res) => {
	try {
		...
		const formattedResult = result.records.reduce((acc, { _fields }) => {
			...
			const completeSources = sources.reduce(
...
const getWithSources = async (req, res) => {
	try {
		...
		if (result.records.length === 0) {
			return res
				.status(404)
				.end(`SAR or Erasure request ${req.params.id} does not exist`);
		}
		...
		const formattedResult = Object.assign({}, request, {
