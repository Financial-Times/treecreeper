...
const send = stub();
const end = stub();
const {emptySubmissions, submissionsToCloneFrom} = require('./fixtures/supplier.fixtures.js');
...
describe('Supplier - API endpoints', () => {
    describe('create', () => {
        ...
        it('should clone supplier diligence', async() => {
            dbRun.onCall(0).resolves('successfully created supplier, contracts and submissions');
            ...
            await supplier.create(req, res);
            expect(res.status).to.be.calledWith(200);
            expect(send).to.be.calledWith('successfully created supplier, contracts and submissions');    
        });
    });
});
