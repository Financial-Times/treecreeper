...
describe('Event log writer', () => {
	...
	it('should add legacy attributes to a given node for backwards compatibility with CMDB', async () => {
		...
		await eventLogWriter.sendEvent(Object.assign({}, defaultEvent, {
			type: givenType,
			code: givenCode
		}));

		const call = stubKinesis.putRecord.getCall(0).args[0]
		expect(call).to.have.property('key', `${givenType}/${givenCode}`)
	...
	it('should add the code in lower case', async () => {
		const givenValue = 'dummySystemCode';
		await eventLogWriter.sendEvent(Object.assign({}, defaultEvent, {
			code: givenValue
		}));

		expect(stubKinesis.putRecord.getCall(0).args[0].code).to.equal('dummysystemcode');
	});
