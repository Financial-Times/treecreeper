...
const validateAttribute = (typeName, attributeName, attributeValue) => {
	...
	if (!attributeDefinition) {
		throw new BizOpsError(
			`Invalid property \`${attributeName}\` on type \`${typeName}\`.`
		);
	}
	...
	const { validator, relationship } = attributeDefinition;
	const type =
		primitiveTypesMap[attributeDefinition.type] || attributeDefinition.type;

	if (type === 'Boolean') {
		if (typeof attributeValue !== 'boolean') {
			throw new BizOpsError(
				`Invalid value \`${attributeValue}\` for property \`${attributeName}\` on type \`${typeName}\`.
			Must be a Boolean`
			);
		}
	} else if (type === 'Float') {
		...
		if (
			...
			Math.round(attributeValue) !== attributeValue
		) {
			throw new BizOpsError(
				`Invalid value \`${attributeValue}\` for property \`${attributeName}\` on type \`${typeName}\`.
		...
		toArray(attributeValue).map(value =>
			validateAttribute(type, 'code', value)
		);
	}
};
...
const validateAttributes = (typeName, attributes) =>
	Object.entries(attributes).forEach(([propName, value]) =>
		validateAttribute(typeName, propName, value)
	);
...
const validateAttributeNames = attributes =>
	Object.keys(attributes).map(validateAttributeName);
