...
const validateRelationship = ({
	...
	let candidates = getType(nodeType, { relationships: 'flat' }).relationships;
		...
		([propName, { pattern, type }]) => {
			if (propName in attributes) {
				...
				if (type === 'Boolean') {
					...
					pattern &&
						console.log({ propName, pattern, type, theType: typeSchema.name });
					...
					const validVals = Object.values(enumsSchema[type].options);
